-- Provider choice and document-sharing consent are separate decisions.
-- A creator may select/contact a provider without authorising Zeke to share.
...
begin
  ...
  set creator_path = 'legal',
      ...
      share_with_provider_consent = coalesce(p_share_consent, false),
  ...
  values (
    ...
    'Creator selected ' || provider_name || ' for direct contact.'
      || case when coalesce(p_share_consent, false)
        then ' Creator also authorised future coordination of records marked for sharing after direct engagement.'
        else ' No provider-sharing consent was given at this stage.'
      end
  );
