...
- Fix, in `components/marketing/Hero.tsx`, all changes `lg:` prefixed so **mobile and tablet are provably untouched**: section padding `lg:pt-32 lg:pb-20` -> `lg:pt-16 lg:pb-12`; benefits grid gained `lg:mt-6` (from `mt-7`); desktop CTA row gained `lg:mt-6` (from `mt-8`).
...
### Cloudflare migration COMPLETE (zekesolution.com now behind Cloudflare): 2026-08-15

- IN PROGRESS. The Cloudflare zone is built and verified but `zekesolution.com` is still served by Namecheap nameservers (`dns1/dns2.registrar-servers.com`). Nothing about the live site or mail has changed yet.
- Cloudflare account `zeke-solution`, id `72fe50a8344078e739c4b55818d9e3dd`. Zone `zekesolution.com`, id `b6c2564baf932c538c3a5040f87ddff6`, **Free** plan, status `pending`. Assigned nameservers: `ariadne.ns.cloudflare.com` and `ivan.ns.cloudflare.com`.
- The live Namecheap zone was inventoried first: 7 records, of which 4 carry mail (Google Workspace MX, Resend SPF on `send`, Resend bounce MX on `send`, Resend DKIM at `resend._domainkey`). Losing any of those at cutover would break company email and every Zeke transactional email. Cloudflare's auto-scan imported all 7 correctly, with only the apex A and www CNAME proxied and every mail record left DNS-only.
- Two pre-existing gaps were found and fixed while the zone was still inert: the root domain had **no SPF record** (Google Workspace mail was unauthenticated) and there was **no DMARC record at all**. Added `v=spf1 include:_spf.google.com include:amazonses.com ~all` on the apex and `v=DMARC1; p=none; rua=mailto:mufeed@zekesolution.com; fo=1` on `_dmarc`. DMARC is deliberately monitoring-only; do not tighten to quarantine or reject until the reports have been read.
- All 9 records were confirmed by querying `ariadne` and `ivan` directly, not just read back from the API.
- Ops scripts live in `C:\Users\SEO EXECUTIVE\AppData\Local\ZekeOps\` (deliberately outside any git repo, since the API token sits beside them): `cf-verify-records.ps1` (read-only pre-cutover diff), `cf-add-mail-records.ps1` (idempotent), `cf-verify-via-dns.ps1` (credential-free check against the assigned nameservers), `cf-set-token.ps1`.
- **Trap that cost most of a session:** newer Cloudflare tokens have a `cfat_` prefix, are account-owned, and return HTTP 401 code 1000 "Invalid API Token" against the old `/user/tokens/verify` endpoint even when completely valid. Use `/accounts/{account_id}/tokens/verify`. The failure is indistinguishable from a bad paste, which sent us through five needless token recreations.
- Security settings already applied to the pending zone, so they activate the moment nameservers move: SSL/TLS `strict` (it was `full`; `flexible` would have caused an infinite redirect loop with Vercel), Always Use HTTPS `on`, minimum TLS `1.2`. The Cloudflare Managed Free Ruleset and DDoS L7 ruleset are active by default, and Bot Fight Mode is confirmed `off` (it breaks auth callbacks - leave it off).
- Rate limiting is live on `/login`, `/register`, `/reset`, `/verify`, `/update-password` and `/auth/*`, action block. **Free plan permits only a 10-second window** (`period 60` is rejected with "not entitled to use the period 60, can only use a period among [10]"), so the rule is 10 requests per 10 seconds per IP with a 10-second block. This stops burst brute force but NOT slow credential stuffing paced under 1 req/sec. Longer windows require Pro.
- No Page Rules exist, so nothing is caching HTML. Never add a "Cache Everything" rule: Zeke serves role-based signed-in pages, and edge-caching HTML could serve one user's dashboard to another.
- CUTOVER DONE 2026-08-15: nameservers moved at Namecheap, delegation confirmed via a public resolver, Cloudflare zone status `active`.
- **Brief HTTPS outage at cutover, and the lesson from it.** The apex and www were proxied when the zone activated, but Cloudflare's Universal SSL certificate was still `pending_validation`, so every HTTPS request hit a Cloudflare edge with no certificate and got a fatal TLS alert. The site was unreachable over HTTPS until both records were set to DNS-only, after which it returned 200 immediately from Vercel's own certificate. **Next time, cut over with the web records grey-clouded and only enable the proxy once the edge certificate reports `active`.**
- The Universal SSL certificate went `active` shortly afterwards and both web records were re-proxied. **Migration is COMPLETE and Cloudflare is now in the request path.**
- Final verification passed: apex returns 200 and `www` returns 308 to the apex then 200, both with `Server: cloudflare` and a `CF-RAY` header confirming edge delivery. The app's own security headers survive the proxy intact (CSP with `frame-ancestors 'none'`, HSTS `max-age=63072000`). `/creator`, `/brand` and `/admin` return 307 to `/login`. `/login`, `/robots.txt`, `/sitemap.xml` and `/manifest.webmanifest` return 200. All mail records still resolve: Google Workspace MX, Resend bounce MX, Resend DKIM, and the new DMARC.
- REMAINING: send one real Resend email end to end (signup confirmation or password reset) to prove deliverability under the new SPF/DMARC records - this has NOT been tested, only the DNS records were verified. Read the DMARC reports arriving at mufeed@zekesolution.com for a couple of weeks before considering `p=quarantine`. Roll or delete the Cloudflare API token, whose value was exposed in a session transcript.
- Observation, not yet a problem: `CF-RAY` showed CDG/FRA/HKG colos for requests from the UAE, while Vercel functions run in `sin1`. Free plan routing is not always geographically optimal. Worth measuring real latency before assuming Cloudflare improved performance; its value here is the WAF and rate limiting, not speed.
- The API token used for this work must be rolled or deleted once the migration is complete: its value was exposed in a session transcript.
