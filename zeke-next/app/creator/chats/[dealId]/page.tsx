...
export default async function CreatorChatPage({ params }: { params: Promise<{ dealId: string }> }) {
  ...
  const { data: deal } = await supabase
    .from("deals")
    .select("title,amount,status,influencer_id,creator_chat_closed_at,brand:profiles!deals_brand_id_fkey(display_name)")
    .eq("id", dealId)
    .single();
  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-3 border-b border-border pb-4">
        <Link href="/creator/chats" className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-[10px] border border-border bg-white/5 text-light">
          <BackIcon />
        </Link>
        ...
        {deal.status === "negotiating" ? (
          <Link href="/creator/offers" className="flex-shrink-0 text-[11px] font-semibold text-gold">Offer Inbox</Link>
        ) : (
          <Link href={`/creator/deals/${dealId}`} className="flex-shrink-0 text-[11px] font-semibold text-muted">View accepted deal</Link>
        )}
      </div>
      {deal.status === "completed" && (
        <CreatorChatControl dealId={dealId} initiallyClosed={Boolean(deal.creator_chat_closed_at)} />
      )}
