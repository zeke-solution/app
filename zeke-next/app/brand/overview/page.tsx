...
export default async function BrandOverviewPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-black text-white">Welcome, {session.profile.display_name}</h1>
      <p className="mb-5 mt-1 text-sm text-muted">
        {[session.brand?.brand_type, session.profile.location].filter(Boolean).join(" · ")}
      </p>

      <StatGrid>
        <StatCard icon={<CampaignIcon width={18} height={18} />} iconColor="#E94560" valueColor="#E94560" value={activeCampaignsRes.count ?? 0} label="Campaigns" />
        <StatCard
      ...
      <div className="mb-3 flex items-center justify-between">
        <div className="text-sm font-bold text-white">Active Campaigns</div>
      ...
      <div className="mb-3 flex items-center justify-between">
        <div className="text-sm font-bold text-white">Recommended Creators</div>
