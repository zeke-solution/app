...
- Current refinement supersedes neutral hairline borders: dashboard content cards, inset content boxes, full-width data rows, and their neutral separators are borderless. Use light-canvas/white-surface contrast, spacing, and restrained shadow for hierarchy. Keep input fields, ordinary action buttons, tabs, semantic status/error borders, and destructive controls visibly bounded.
...
- Password recovery passed a full production proof on 2026-08-11: Resend delivered the current template, its redacted link targeted Zeke `/auth/confirm` with `token_hash` and `type`, a separate session verified it, the password updated, and the new password signed in. The temporary user was deleted. If an owner still sees the old Supabase `/auth/v1/verify` error, Resend timestamps should be checked first: a previously delivered email cannot adopt the new template, so request exactly one fresh message and use only that one.
