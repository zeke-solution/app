  ...
  if (status === "active") {
    return (
      <div className="flex items-center justify-between gap-3 rounded-xl border border-gold/40 bg-gold/[0.08] p-3.5">
        <div>
          <div className="text-[13px] font-bold text-gold">&#128737; Get Zeke Shield</div>
          <div className="mt-0.5 text-xs text-muted">You are a Shield member. Thank you for backing Zeke.</div>
        </div>
        <Button variant="gold" size="sm" disabled className="opacity-70">
          &#128737; Active
        </Button>
      </div>
    );
      ...
      <div>
        ...
        <div className="mt-0.5 text-xs text-muted">
          {status === "pending"
            ...
            : "Legal protection + gold badge + priority in searches"}
        </div>
      </div>
